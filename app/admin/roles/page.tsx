...
export default function RolesPage() {
  ...
  const fetchRoles = async () => {
    setLoading(true);
    try {
      const response = await mcApiService.get("/roles/system");
      const list = response.roles || response.data || response || [];
      setRoles(list);
    } catch (error) {
  ...
  const fetchAvailablePermissions = async () => {
    try {
      const response = await mcApiService.get("/d/permission");
      setAvailablePermissions(response.permissions || response.data || response || []);
    } catch (error) {
  ...
  const fetchRolePermissions = async (roleId: string): Promise<Permission[]> => {
    try {
      const response = await mcApiService.get(`/roles/${roleId}/permissions`);
  ...
  const handleAddRole = async () => {
    ...
    try {
      ...
      const response = await mcApiService.post("/roles", role);
      const roleId = response.role?.id || response.data?.id || response.id;
      if (!roleId) throw new Error("No role id returned");

      if (selectedPermissionIds.length > 0) {
        await mcApiService.post(`/roles/${roleId}/permissions`, {
  ...
  const handleEditRole = async (role: Role) => {
    setCurrentRole(role);
    setNewRole({ name: role.name, description: role.description, isSystemRole: !!role.isSystemRole });

    // fetch full Permission objects for the role
    const permissions = await fetchRolePermissions(role.id);
    const permissionIds = permissions.map((p) => p.id);

    // merge any permissions not present in availablePermissions so they show up in the list
    setAvailablePermissions((prev) => {
      const map = new Map<string, Permission>(prev.map((p) => [p.id, p]));
  ...
  const handleUpdateRole = async () => {
    ...
    try {
      ...
      await mcApiService.patch(`/roles/${currentRole.id}`, updatedRole);

      // get current permission ids from Permission[] result
      const currentPermissionIds = (await fetchRolePermissions(currentRole.id)).map((p) => p.id);
      ...
      if (permissionsToAdd.length > 0) {
        await mcApiService.post(`/roles/${currentRole.id}/permissions`, {
          permissionIds: permissionsToAdd,
        });
      }

      if (permissionsToRemove.length > 0) {
        await mcApiService.delete(`/roles/${currentRole.id}/permissions`, {
          body: { permissionIds: permissionsToRemove },
        });
      }
  ...
  const handleDeleteRole = async () => {
    ...
    try {
      await mcApiService.delete(`/roles/${currentRole.id}`);
  ...
  const columns: ColumnDef<Role>[] = [
    ...
    {
      id: "isSystemRole",
      accessorFn: (row) => (row.isSystemRole ? "Yes" : "No"),
      header: "System Role",
      cell: ({ row }: { row: { original: Role } }) => {
        const isSystem = !!row.original.isSystemRole;
        return (
          <span className={isSystem ? "text-sm font-medium text-green-700" : "text-sm text-muted-foreground"}>
            {isSystem ? "Yes" : "No"}
          </span>
        );
      },
  ...
  return (
    <div className="p-4 md:p-10 mx-auto max-w-7xl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Roles (System)</h1>
        <Button
          onClick={() => {
            setCurrentRole(null);
            setNewRole({ name: "", description: "", isSystemRole: false });
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setNewRole({ ...newRole, isSystemRole: !!checked })}
                />
                <Label htmlFor="isSystemRole" className="cursor-pointer">System role</Label>
              </div>
            </div>
